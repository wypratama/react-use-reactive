...
- **Unsupported shapes fail loudly.** Accessor or non-enumerable structural properties (except the standard array `length`), `__proto__` as a structural key, `Object.defineProperty`/`defineProperties`, and `Object.preventExtensions`/`freeze`/`seal` all throw a clear `TypeError` before corrupting state; the tree stays usable afterwards. `delete arr.length` throws the language-level invariant `TypeError`. A rejected single-property assignment never partially attaches; multi-step natives (`Object.assign`, `splice`, `fill`) apply sequentially, so earlier writes may already have applied before a later value throws — the tree stays internally valid.
